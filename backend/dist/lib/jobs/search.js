...
import { collapseDuplicateRankedJobs, scoreJobForSearch, } from "./discovery.js";
import { buildJobSemanticContent } from "../rag/job-documents.js";
import { semanticTextScore } from "../rag/embedding.js";
export const publicJobInclude = Prisma.validator()({
    employer: {
        select: {
            companyName: true,
            location: true,
...
function boostSemanticRanking(results, query) {
    if (!query?.trim()) {
        return results;
    }
    return results
...
export function buildJobSearchWhere(filters) {
    const conditions = [
...
export async function loadCandidatePreferenceContext(req) {
...
export function buildPreferenceContext(filters, base) {
    return {
        ...base,
        ...
        requiresVisaSponsorship: filters.visaSponsorship === "YES" ? true : (base?.requiresVisaSponsorship ?? false),
        prefersRelocationSupport: filters.relocationAssistance ?? base?.prefersRelocationSupport ?? false,
...
export async function fetchRankedJobs(input) {
    ...
    const ranked = boostSemanticRanking(collapseDuplicateRankedJobs(jobs.map((job) => scoreJobForSearch(job, input.preferenceContext))), input.preferenceContext?.query).map((result) => ({
        ...result.job,
