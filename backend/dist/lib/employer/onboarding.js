...
export function normalizeEmployerCandidateFilters(value) {
    const candidateFilters = (value && typeof value === "object") ? value : {};
    return {
        skills: Array.isArray(candidateFilters.skills)
            ? candidateFilters.skills.filter((item) => typeof item === "string").slice(0, 12)
            : [],
...
export function hasCandidateFilters(filters) {
    return Boolean(filters.skills.length > 0 ||
        filters.location ||
        filters.minExperience !== null ||
        filters.verifiedOnly ||
        filters.visaPreference === "SPONSORED_ONLY");
}
export function buildEmployerOnboardingChecklist(input) {
    return [
        {
            key: "company_setup",
...
export function completionPercentage(checklist) {
    if (checklist.length === 0)
        return 0;
    return Math.round((checklist.filter((item) => item.done).length / checklist.length) * 100);
}
export function nextEmployerAction(checklist) {
...
export function verificationIsStrong(level) {
    switch (level) {
...
export function qualifiedApplicantsCount(statuses) {
...
export function timeToFirstQualifiedApplicantHours(input) {
    ...
    return Number(((input.firstQualifiedShortlistAt.getTime() - input.firstApprovedJobAt.getTime()) / (1000 * 60 * 60)).toFixed(2));
}
export function isPlanSelectionDone(plan, selectedPlan) {
    return Boolean(selectedPlan ||
        (plan && plan !== SubscriptionPlan.FREE && plan !== SubscriptionPlan.EMPLOYER_FREE));
}
export function isJobApproved(status) {
