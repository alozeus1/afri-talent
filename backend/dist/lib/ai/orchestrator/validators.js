...
export function validateAgentOutput(schema, raw, agentName) {
    ...
    console.error(`[validateAgentOutput] ${agentName} output failed schema validation.\n` +
        `First ${issues.length} issue(s):\n` +
        issues
            .map((iss, i) => `  ${i + 1}. [${iss.path.join(".") || "<root>"}] ${iss.message}`)
            .join("\n"));
    throw new Error(`${agentName} output failed schema validation: ${firstIssue}`);
}
