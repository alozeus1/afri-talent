...
import { ResumeSchemaValidator, JobSchemaValidator, MatchSchemaValidator, TailoredResumeSchemaValidator, CoverLetterPackSchemaValidator, GuardReportSchemaValidator, } from "./validators.js";
// ── Mock AI mode ─────────────────────────────────────────────────────────────
...
function mockMatchForJob(jobId) {
    return {
        score: 75,
...
function mockTailoredForJob(jobId) {
    return {
        tailored_resume: {
 ...
 */
function truncateOutput(out, runId) {
    ...
    for (const t of out.tailored_outputs) {
        ...
        if (tr.ats_keywords.length > MAX_ATS_KW)
            tr.ats_keywords = tr.ats_keywords.slice(0, MAX_ATS_KW);
        if ((tr.change_log?.length ?? 0) > MAX_CHANGE_LOG)
            tr.change_log = tr.change_log.slice(0, MAX_CHANGE_LOG);
...
export async function runOrchestrator(input) {
    ...
    if (MOCK_AI) {
        ...
        const mockJobJson = {
            ...
            visa_sponsorship: "UNKNOWN", relocation_assistance: false, eligible_countries: [],
            description: "Mock job description", requirements: [], responsibilities: [],
        };
        const rankedJobs = inputJobs.map((j) => ({
            job_id: j.job_id,
        ...
        // apply_pack
        const tailoredOutputs = rankedJobs.map((rj) => ({
            job_id: rj.job_id,
    ...
    const notes = [];
    let status = "ok";
    logger.info({ run_id, user_id, run_type, job_count: jobs.length, token_budget: tokenBudget }, "[orchestrator] run started");
    // ── Step 1: Parse resume ────────────────────────────────────────────────────
    let resumeJson;
    ...
    if (run_type === "resume_review") {
        ...
        const out = { run_id, status, budget: budget.toInfo(), resume_json: resumeJson, ranked_jobs: [], tailored_outputs: [], notes_for_ui: notes };
    ...
    if (run_type === "job_match") {
        ...
        const out = { run_id, status, budget: budget.toInfo(), resume_json: resumeJson, ranked_jobs: rankedJobs, tailored_outputs: [], notes_for_ui: notes };
        truncateOutput(out, run_id);
        return out;
    }
    ...
    logger.info({ run_id, user_id, ranked: rankedJobs.length, tailored: tailoredOutputs.length, tokens_used: budget.used }, "[orchestrator] run complete (apply_pack)");
    const out = {
        run_id,
        status,
...
async function tailorJob({ jobId, resume, job, budget, notes, runId, userId }) {
    for (let attempt = 1; attempt <= 2; attempt++) {
        try {
            ...
            notes.push(`job ${jobId}: tailoring error (attempt ${attempt}) — ${err.message}`);
            return null;
        }
    }
