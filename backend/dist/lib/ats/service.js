import { ATSApplicationLinkStatus, ATSConnectionStatus, ATSHealthStatus, ATSSyncStatus, ATSWebhookEventStatus, JobStatus, Prisma, } from "@prisma/client";
...
import { defaultStageLabelForApplicationStatus, fetchAtsJobs, getAtsProviderCapabilities, mapExternalStageToApplicationStatus, normalizeAtsWebhookPayload, providerLabel, pushAtsCandidateStageUpdate, verifyAtsWebhookSignature, } from "./providers.js";
...
function getConnectionMetadata(metadata) {
...
function getStageMappings(metadata) {
...
function buildConnectionBaseUrl(baseUrl, provider, connectionId) {
    return `${baseUrl.replace(/\/$/, "")}/api/ats/webhooks/${provider}/${connectionId}`;
}
function serializeConnection(connection, baseUrl, recentLogSummary) {
...
export async function refreshConnectionHealth(connectionId) {
...
export async function listEmployerAtsConnections(employerId, baseUrl) {
...
export async function getEmployerAtsDashboard(employerId, baseUrl) {
    const connections = await listEmployerAtsConnections(employerId, baseUrl);
    const providerBreakdown = Object.values(connections.reduce((acc, connection) => {
        ...
        current.total += 1;
        if (connection.healthStatus === ATSHealthStatus.HEALTHY)
            current.healthy += 1;
        if (connection.healthStatus === ATSHealthStatus.DEGRADED)
            current.degraded += 1;
        if (connection.healthStatus === ATSHealthStatus.DOWN)
            current.down += 1;
        if (connection.healthStatus === ATSHealthStatus.NEEDS_ATTENTION)
...
export async function runAtsConnectionTest(params) {
    ...
    try {
        const jobs = await withRetry(() => fetchAtsJobs({
            provider: connection.provider,
            externalOrgId: connection.externalOrgId,
...
function parseExternalJobId(sourceId, provider) {
    if (!sourceId) {
        return null;
    }
...
export async function maybeCreateAtsApplicationLink(applicationId) {
    ...
    const linkStatus = connection.twoWaySyncEnabled && Object.keys(stageMappings).length > 0
...
export async function syncApplicationStatusToAts(params) {
    ...
    try {
        const result = await withRetry(() => pushAtsCandidateStageUpdate({
            provider: connection.provider,
            externalOrgId: connection.externalOrgId,
            accessToken: decryptString(connection.encryptedAccessToken),
        ...
        if (!result.ok) {
            ...
            await prisma.aTSSyncRun.update({
                ...
                data: {
                    status: ATSSyncStatus.PARTIAL,
                    finishedAt: new Date(),
                    warnings: [{ message: result.message }],
                },
            });
            await createAtsAuditLog({
                connectionId: connection.id,
            ...
            if (!link) {
                ...
                await prisma.aTSSyncRun.update({
                    ...
                    data: {
                        status: ATSSyncStatus.PARTIAL,
                        finishedAt: new Date(),
                        warnings: [{ message: "No matching ATS application link found." }],
                    },
                });
                return {
                    duplicate: false,
...
export async function getAdminAtsDashboard(baseUrl) {
    ...
    const serializedConnections = await Promise.all(connections.map(async (connection) => {
        const [failedSyncRuns, failedWebhookEvents, pendingApplicationLinks] = await Promise.all([
            prisma.aTSSyncRun.count({
    ...
    return {
        ...
        providerBreakdown: Object.values(serializedConnections.reduce((acc, connection) => {
