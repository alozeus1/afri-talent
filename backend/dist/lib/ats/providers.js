...
function detectVisaSponsorship(text) {
    ...
    if (/visa sponsorship|sponsor visa|work permit support/.test(lower))
        return "YES";
    if (/no visa sponsorship|cannot sponsor|must be authorized to work/.test(lower))
        return "NO";
    return "UNKNOWN";
}
function detectSeniority(title, description) {
    const bag = `${title} ${description}`.toLowerCase();
    if (/staff|principal|lead/.test(bag))
        return "Lead";
    if (/senior|sr\.?/.test(bag))
        return "Senior";
    if (/junior|jr\.?|entry/.test(bag))
        return "Junior";
    if (/director|vp|head of|chief/.test(bag))
        return "Executive";
    return "Mid-level";
}
function providerAuthHeader(token) {
    return `Basic ${Buffer.from(`${token}:`).toString("base64")}`;
}
function bearerOrBasicLeverAuthHeader(token) {
...
function getNestedString(source, candidatePaths) {
...
export function getAtsProviderCapabilities(params) {
    ...
    const stageWritebackReady = Boolean(params.twoWaySyncEnabled)
...
async function fetchLever(siteToken) {
    const response = await fetch(`https://api.lever.co/v0/postings/${encodeURIComponent(siteToken)}?mode=json`, {
        headers: {
            Accept: "application/json",
            "User-Agent": "AfriTalent/1.0",
        },
...
async function fetchWorkable(accountSlug, accessToken) {
    if (!accessToken) {
...
export function normalizeAtsWebhookPayload(params) {
    ...
    const eventType = (rawHeaderEvent
        || getNestedString(body, [["action"], ["type"], ["event"], ["trigger"], ["event_type"]])
        || "unknown").trim();
    const eventKey = (Array.isArray(params.headers["greenhouse-event-id"]) ? params.headers["greenhouse-event-id"][0] : params.headers["greenhouse-event-id"])
    ...
    const isStageChangeEvent = /stage/i.test(eventType)
        || /candidate_stage_change|application_updated|candidate_moved/i.test(lowerEvent)
        || stageName !== null;
    const shouldTriggerJobSync = /job/i.test(lowerEvent)
...
export function mapExternalStageToApplicationStatus(stageName) {
    if (!stageName) {
        return null;
    }
...
export function defaultStageLabelForApplicationStatus(status) {
    switch (status) {
...
async function pushGreenhouseStageUpdate(params) {
    ...
    const performAsUserId = typeof metadata.performAsUserId === "string" && metadata.performAsUserId.trim().length > 0
    ...
    const response = await fetch(`https://harvest.greenhouse.io/v1/applications/${encodeURIComponent(params.externalApplicationId)}/move`, {
        method: "POST",
        headers: {
            Accept: "application/json",
...
async function pushLeverStageUpdate(params) {
    if (!params.accessToken) {
        return {
            ok: false,
            mode: "manual",
    ...
    const response = await fetch(`https://api.lever.co/v1/opportunities/${encodeURIComponent(params.externalApplicationId)}/stage`, {
        method: "PUT",
        headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
...
async function pushWorkableStageUpdate(params) {
    if (!params.accessToken) {
        return {
            ok: false,
            mode: "manual",
    ...
    const metadata = params.metadata ?? {};
    const memberId = typeof metadata.performAsUserId === "string" && metadata.performAsUserId.trim().length > 0
        ? metadata.performAsUserId.trim()
        : null;
    if (!memberId) {
        return {
    ...
    const response = await fetch(`https://${params.externalOrgId}.workable.com/spi/v3/candidates/${encodeURIComponent(params.externalCandidateId)}/move`, {
        method: "POST",
        headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
...
export async function pushAtsCandidateStageUpdate(params) {
    switch (params.provider) {
        case "GREENHOUSE":
