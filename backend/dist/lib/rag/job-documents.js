...
export const semanticJobSelect = Prisma.validator()({
    id: true,
    title: true,
    description: true,
    location: true,
...
export function buildJobSemanticContent(job) {
    const segments = [
        job.title,
        job.description,
...
export function buildJobSemanticMetadata(job) {
    return {
        jobId: job.id,
        title: job.title,
        location: job.location,
...
export function isSemanticJobIndexable(job) {
    return job.status === JobStatus.PUBLISHED && !job.isExpired;
}
//# sourceMappingURL=job-documents.js.map