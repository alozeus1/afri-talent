import { ApplicationStatus, CandidateLifecycleTriggerType, CandidateVerificationLevel, NotificationType, Role, } from "@prisma/client";
import prisma from "./prisma.js";
import { buildJobSearchWhere, buildPreferenceContext, fetchRankedJobs } from "./jobs/search.js";
...
function hasCitizenLikeVisaStatus(visaStatus) {
...
function buildExperimentVariant(userId, key, variants) {
    let total = 0;
...
export function evaluateLifecycleDelivery(input) {
    ...
    const notificationsThisWeek = weekWindowStartedAt && weekWindowStartedAt.getTime() === currentWeek.getTime()
    ...
    if (input.state.lastNotificationAt) {
        const hoursSinceLastNotification = (now.getTime() - input.state.lastNotificationAt.getTime()) / (1000 * 60 * 60);
...
export function buildCandidateRetentionExperiments(userId) {
    return [
        {
            key: "digest_hero_v1",
    ...
    const journeys = [
        {
            ...
            description: input.savedSearchCount > 0
        ...
        },
        {
            ...
            description: input.profileCompleteness >= 85
        ...
        },
        {
            ...
            title: input.verificationLevel === CandidateVerificationLevel.SKILLS_VERIFIED ||
            ...
            description: input.verificationLevel === CandidateVerificationLevel.SKILLS_VERIFIED ||
                input.verificationLevel === CandidateVerificationLevel.EMPLOYMENT_HISTORY_PARTIALLY_VERIFIED
            ...
            status: input.verificationLevel === CandidateVerificationLevel.SKILLS_VERIFIED ||
                input.verificationLevel === CandidateVerificationLevel.EMPLOYMENT_HISTORY_PARTIALLY_VERIFIED
                ? "DONE"
                : "READY",
            href: "/candidate/trust",
            ctaLabel: input.verificationLevel === CandidateVerificationLevel.SKILLS_VERIFIED ||
                input.verificationLevel === CandidateVerificationLevel.EMPLOYMENT_HISTORY_PARTIALLY_VERIFIED
                ? "Review trust profile"
                : "Boost trust score",
            priority: input.verificationLevel === CandidateVerificationLevel.SKILLS_VERIFIED ||
                input.verificationLevel === CandidateVerificationLevel.EMPLOYMENT_HISTORY_PARTIALLY_VERIFIED
                ? 7
                : 3,
        },
        {
            ...
            description: input.openApplications > 0
...
async function ensureCandidateNotificationPreferences(userId) {
...
async function ensureCandidateLifecycleState(userId) {
...
async function normalizeLifecycleStateForNow(userId, state, now = new Date()) {
    const currentWeek = startOfWeek(now);
...
async function loadCandidateRetentionContext(userId) {
    const [user, preferences, state, profile, trustProfile, savedSearches, applications, lastInterviewSession] = await Promise.all([
...
function buildRecommendationFilters(context) {
    ...
    const salaryMin = context.savedSearches
        .map((search) => search.salaryMin)
        .filter((value) => typeof value === "number")
        .sort((left, right) => right - left)[0] ?? null;
    const salaryMax = context.savedSearches
        .map((search) => search.salaryMax)
        .filter((value) => typeof value === "number")
        .sort((left, right) => left - right)[0] ?? null;
    const remote = context.savedSearches.some((search) => search.remoteOnly);
    const requiresVisaSponsorship = context.savedSearches.some((search) => search.visaSponsorship) ||
...
export async function getCandidateRecommendationFeed(userId, limit = 8) {
...
async function buildWeeklyDigestPreviewFromRecommendations(recommendations) {
    ...
    return {
        ...
        freshnessWindowLabel: jobs.length > 0 && jobs.every((job) => job.discovery.freshnessLabel === "FRESH" || job.discovery.freshnessLabel === "RECENT")
            ? "Fresh this week"
            : "Mixed freshness",
        jobs,
    };
}
async function loadSalaryInsight(userId, profile) {
...
export async function getCandidateRetentionSummary(userId) {
    const context = await loadCandidateRetentionContext(userId);
    if (!context) {
        return null;
    }
    ...
    const openApplications = context.applications.filter((application) => application.status === ApplicationStatus.PENDING ||
        application.status === ApplicationStatus.REVIEWING ||
        application.status === ApplicationStatus.SHORTLISTED).length;
    const notificationBudgetRemaining = Math.max(0, context.preferences.maxNotificationsPerWeek - context.state.notificationsThisWeek);
    const verificationLevel = context.trustProfile?.verificationLevel ?? CandidateVerificationLevel.UNVERIFIED;
...
function buildLifecycleDedupeKey(triggerType, userId, now) {
...
export async function runCandidateRetentionForUser(userId, now = new Date()) {
    const context = await loadCandidateRetentionContext(userId);
    ...
    const weeklyDigest = await buildWeeklyDigestPreviewFromRecommendations(recommendations);
    const salaryInsight = await loadSalaryInsight(userId, context.profile);
    const activeApplications = context.applications.filter((application) => application.status === ApplicationStatus.REVIEWING ||
        application.status === ApplicationStatus.SHORTLISTED);
    const mostRecentApplication = context.applications[0];
    const noRecentApplications = !mostRecentApplication ||
        mostRecentApplication.createdAt < daysAgo(APPLICATION_REMINDER_LOOKBACK_DAYS, now);
    if (context.preferences.weeklyDigests &&
    ...
    if (context.preferences.interviewPrepRecommendations &&
    ...
    if (context.preferences.visaRelocationAlerts &&
    ...
    if (context.preferences.applicationReminders &&
    ...
    if (context.preferences.profileCompletionNudges &&
    ...
    if (context.preferences.verificationCompletionNudges &&
    ...
    if (context.preferences.salaryInsights &&
        salaryInsight &&
...
export async function runCandidateRetentionCycle(now = new Date()) {
