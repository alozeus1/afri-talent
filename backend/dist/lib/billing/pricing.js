...
function getFallbackRegionalPrice(plan, region, interval, currency) {
    const normalizedCurrency = currency?.toUpperCase();
    return DEFAULT_REGIONAL_PRICE_CATALOG.find((row) => (row.plan === plan
        && row.region === region
        && row.interval === interval
        && (!normalizedCurrency || row.currency === normalizedCurrency))) ?? null;
}
function getFallbackRegionalPrices(region, currency) {
    const normalizedCurrency = currency?.toUpperCase();
    return DEFAULT_REGIONAL_PRICE_CATALOG.filter((row) => (row.region === region
        && (!normalizedCurrency || row.currency === normalizedCurrency)));
}
function toPriceInfo(price) {
    const stripePriceId = price.stripePriceId ?? resolveStripeCatalogPriceId(price.plan, price.region, price.interval, price.currency);
    return {
        plan: price.plan,
        region: price.region,
        ...
        amount: price.amount,
        stripePriceId,
        flutterwavePlanId: resolveFlutterwaveCatalogPlanId(price.plan, price.region, price.interval, price.currency),
 ...
 */
export async function getRegionalPrice(plan, region, interval = BillingInterval.MONTHLY, currency) {
    const where = {
        plan,
        region,
        interval,
 ...
 */
export async function getRegionalPrices(region, currency) {
    const where = {
        region,
        isActive: true,
    };
 ...
 */
export async function resolveStripePriceId(userId, plan, interval = BillingInterval.MONTHLY) {
...
export async function resolveCheckoutPrice(userId, plan, interval = BillingInterval.MONTHLY) {
    const profile = await prisma.userBillingProfile.findUnique({ where: { userId } });
    const region = profile?.region ?? BillingRegion.ROW;
    const currency = profile?.currency ?? "USD";
