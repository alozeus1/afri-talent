...
function extractName(text) {
    ...
    if (firstLine.length > 80)
        return undefined;
    if (/resume|curriculum|cv/i.test(firstLine))
        return undefined;
    if (/\d|@/.test(firstLine))
        return undefined;
    return firstLine;
...
function extractWorkHistory(workSection) {
    if (!workSection)
        return [];
...
function extractEducation(educationSection) {
    if (!educationSection)
        return [];
...
function extractCertifications(certSection) {
    if (!certSection)
        return [];
    return certSection
        .split(/\n|,|•|·/)
...
function inferYearsExperience(workHistory) {
    const years = new Set();
...
function inferTargetRoles(workHistory, summary) {
    const bag = `${summary || ""}\n${workHistory
...
export async function extractResumeText(fileBuffer, mimetype, fileName) {
...
export function toCandidateProfileDraft(parsed) {
