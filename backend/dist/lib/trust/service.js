import { AbuseReportReason, AbuseReportStatus, CandidatePartnerMarkerStatus, CandidateSkillVerificationMethod, CandidateSkillVerificationStatus, CandidateVerificationLevel, EmployerVerificationLevel, Role, SubscriptionPlan, TrustRiskLevel, VerificationArtifactStatus, VerificationArtifactType, } from "@prisma/client";
import prisma from "../prisma.js";
import { assessCandidateTrust, assessEmployerTrust, candidateBadgeLabel, employerBadgeLabel, riskLevelForScore, } from "./risk.js";
import { domainFromEmail, normalizeDomain } from "./throwaway-domains.js";
function startOfWindow(hours) {
    return new Date(Date.now() - hours * 60 * 60 * 1000);
}
function restrictionFromRisk(level) {
...
function countStructuredEntries(value, requiredKeys) {
...
function isWorkHistoryConsistent(value) {
...
function buildCandidateExplainabilitySignals(input) {
    return [
        ...
        },
        {
            ...
            status: input.profileCompleteness >= 85
                ? "verified"
        ...
        },
        {
            ...
            status: input.hasResume && input.workHistoryEntries > 0 && input.workHistoryConsistent
                ? "verified"
                : input.hasResume || input.workHistoryEntries > 0
                    ? "strengthening"
                    : "needs_attention",
            detail: input.hasResume && input.workHistoryEntries > 0
        ...
        },
        {
            ...
            status: input.educationEvidenceCount > 0 || input.certificationEvidenceCount > 0
                ? "strengthening"
                : "needs_attention",
            detail: input.educationEvidenceCount > 0 || input.certificationEvidenceCount > 0
        ...
        },
        {
            ...
            detail: input.partnerSignalCount > 0
        ...
        },
        {
            ...
            status: input.hasLinkedIn || input.hasGitHub || input.hasPortfolio
                ? "strengthening"
                : "needs_attention",
            detail: input.hasLinkedIn || input.hasGitHub || input.hasPortfolio
...
export async function ensureEmployerTrustProfile(employerId) {
...
export async function ensureCandidateTrustProfile(userId) {
...
export async function refreshEmployerTrustProfile(employerId) {
    await ensureEmployerTrustProfile(employerId);
    const [employer, approvedBusinessDocs, approvedManualReviews, openReports, postingVelocity24h] = await Promise.all([
    ...
    const [subscription, duplicateCompanyNameMatches, duplicateDomainMatches] = await Promise.all([
        ...
        (async () => {
            const lookupDomain = normalizeDomain(employer.website) ?? domainFromEmail(employer.user.email);
            if (!lookupDomain) {
                return 0;
            }
    ...
    const updated = await prisma.employerTrustProfile.update({
        ...
        data: {
            ...
            domainVerifiedAt: assessment.verificationLevel !== EmployerVerificationLevel.UNVERIFIED
                ? employer.trustProfile?.domainVerifiedAt ?? new Date()
                : null,
            businessDocVerifiedAt: approvedBusinessDocs > 0 ? employer.trustProfile?.businessDocVerifiedAt ?? new Date() : null,
            manualApprovedAt: approvedManualReviews > 0 ? employer.trustProfile?.manualApprovedAt ?? new Date() : null,
            premiumTrustedAt: assessment.verificationLevel === EmployerVerificationLevel.PREMIUM_TRUSTED
...
export async function refreshCandidateTrustProfile(userId) {
    const existing = await ensureCandidateTrustProfile(userId);
    const [user, candidateProfile, activeResumeCount, approvedIdentityDocs, approvedCertificationArtifacts, approvedEmploymentProofs, openReports, applicationsLast24h, verifiedSkills, activePartnerMarkers,] = await Promise.all([
    ...
    const assessmentBackedSkillCount = verifiedSkills.filter((skill) => skill.method === CandidateSkillVerificationMethod.ASSESSMENT).length;
    const certificationEvidenceCount = approvedCertificationArtifacts + certificationEntries;
    const hasResume = activeResumeCount > 0;
    const fullyCompletedProfile = (candidateProfile?.profileCompleteness ?? 0) >= 80 &&
        hasResume &&
    ...
    const updated = await prisma.candidateTrustProfile.update({
        ...
        data: {
            ...
            identityVerifiedAt: approvedIdentityDocs > 0 ? existing.identityVerifiedAt ?? new Date() : null,
            skillsVerifiedAt: verifiedSkillCount > 0
                ? existing.skillsVerifiedAt ?? new Date()
                : null,
            employmentVerifiedAt: approvedEmploymentProofs > 0 ? existing.employmentVerifiedAt ?? new Date() : null,
...
export function employerTrustSummary(trustProfile) {
    const warnings = Array.isArray(trustProfile.suspiciousSignals)
        ? trustProfile.suspiciousSignals.filter((item) => typeof item === "string")
        : [];
    return {
...
export function candidateTrustSummary(trustProfile) {
    const warnings = Array.isArray(trustProfile.suspiciousSignals)
        ? trustProfile.suspiciousSignals.filter((item) => typeof item === "string")
        : [];
    const explainability = Array.isArray(trustProfile.explainabilitySignals)
        ? trustProfile.explainabilitySignals.filter((item) => Boolean(item && typeof item === "object" && !Array.isArray(item)))
        : [];
    return {
...
export function jobTrustSummary(input) {
    const employerSummary = input.employerTrustProfile
        ...
        : null;
    const isNewEmployer = !input.employerTrustProfile ||
        input.employerTrustProfile.verificationLevel === EmployerVerificationLevel.UNVERIFIED;
    return {
        riskLevel: input.riskLevel,
        riskScore: input.riskScore,
        jobQualityChecked: Boolean(input.qualityCheckedAt) && input.riskLevel !== TrustRiskLevel.HIGH && input.riskLevel !== TrustRiskLevel.CRITICAL,
        companyReviewed: Boolean(input.employerTrustProfile &&
            [
                EmployerVerificationLevel.MANUAL_REVIEW_APPROVED,
                EmployerVerificationLevel.PREMIUM_TRUSTED,
            ].includes(input.employerTrustProfile.verificationLevel)),
        newEmployerCaution: isNewEmployer,
        publishedRecently: Boolean(input.publishedAt) &&
            input.publishedAt.getTime() >= Date.now() - 7 * 24 * 60 * 60 * 1000,
        employer: employerSummary,
        guidance: input.riskLevel === TrustRiskLevel.HIGH || input.riskLevel === TrustRiskLevel.CRITICAL
...
export function scoreDeltaForReport(reason) {
    switch (reason) {
...
export function trustPriorityForReport(reason) {
    switch (reason) {
        case AbuseReportReason.ADVANCE_FEE_REQUEST:
        case AbuseReportReason.SCAM:
        case AbuseReportReason.IMPERSONATION:
...
export function isPremiumEmployerPlan(plan) {
    return plan === SubscriptionPlan.EMPLOYER_PREMIUM;
}
export function canUseVerifiedCandidateFilter(plan, role) {
