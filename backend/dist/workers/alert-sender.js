...
function hoursSince(from, to = new Date()) {
    if (!from)
...
export async function runAlertDispatchCycle() {
    let sent = 0;
    let errors = 0;
    ...
    const searchIds = Array.from(new Set(pendingAlerts
        .map((alert) => alert.searchId)
        .filter((searchId) => Boolean(searchId))));
    ...
    for (const [userId, userAlerts] of alertsByUser) {
        try {
            ...
            if (eligibleAlerts.length === 1) {
                ...
                const companyName = alert.job.employer?.companyName ?? alert.job.sourceName ?? "Unknown Company";
                ...
                }).catch((err) => logger.warn({ userId: userId.slice(0, 8), err }, "[alert-sender] email failed"));
            }
            else {
                ...
                const topCompany = topAlert.job.employer?.companyName ?? topAlert.job.sourceName ?? "Unknown Company";
                await jobMatchEmail({
                    to: user.email,
                    candidateName: user.name,
                ...
                }).catch((err) => logger.warn({ userId: userId.slice(0, 8), err }, "[alert-sender] digest email failed"));
            }
            ...
            const searchIdsToUpdate = Array.from(new Set(eligibleAlerts
                .map((alert) => alert.searchId)
                .filter((searchId) => Boolean(searchId))));
            ...
            logger.error({ userId: userId.slice(0, 8), alertCount: userAlerts.length, err }, "[alert-sender] failed processing user alerts");
        }
    }
    logger.info({ sent, errors, userCount: alertsByUser.size }, "[alert-sender] dispatch cycle complete");
}
//# sourceMappingURL=alert-sender.js.map