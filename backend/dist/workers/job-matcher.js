...
async function findNewMatchingJobs(search, sinceDate) {
    ...
    return collapseDuplicateRankedJobs(jobs.map((job) => scoreJobForSearch(job, buildPreferenceContext(filters)))).map((result) => ({
        ...result.job,
...
async function matchCandidateProfiles() {
    ...
    for (const profile of orphanProfiles) {
        const rankedJobs = collapseDuplicateRankedJobs(recentJobs.map((job) => scoreJobForSearch(job, {
