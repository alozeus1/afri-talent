...
const CANDIDATE_RETENTION_INTERVAL_MS = parseInt(process.env.CANDIDATE_RETENTION_INTERVAL_HOURS || "12", 10) * 60 * 60 * 1000;
...
export function startScheduler() {
    ...
    intervals.push(setInterval(() => void safeRun("ops-snapshot", runOperationalSnapshotCycle), OPS_SNAPSHOT_INTERVAL_MS));
    intervals.push(setInterval(() => void safeRun("billing-reconciliation", runBillingReconciliationWorker), BILLING_RECONCILIATION_INTERVAL_MS));
    intervals.push(setInterval(() => void safeRun("candidate-retention", runCandidateRetentionWorker), CANDIDATE_RETENTION_INTERVAL_MS));
    intervals.push(setInterval(() => void safeRun("semantic-index", runSemanticIndexWorker), SEMANTIC_INDEX_INTERVAL_MS));
