...
const DEFAULT_CRITICAL_SOURCES = ["GREENHOUSE", "LEVER", "WORKABLE"];
...
function parseConfiguredCriticalSources() {
...
function resolveCriticalSources(enabledSources) {
...
async function getLatestAggregatedJobTimestampFromDb() {
...
export function evaluateIngestionCycle(input) {
    ...
    const allCriticalSourcesFailed = criticalSourceDiagnostics.length > 0 && criticalSourceDiagnostics.every((entry) => entry.status === "failure");
    ...
    const cycleStatus = reasons.length > 0 ? "failure" : "success";
    const successCheckpointEligible = cycleStatus === "success" && diagnostics.sourcesSucceeded > 0 && jobsIngested > 0;
    return {
        cycleStatus,
...
async function persistIngestionReliabilityState(now, evaluation) {
    ...
    return {
        lastSuccessfulIngestionAt: lastSuccessfulIngestionAt ?? ingestionMemoryState.lastSuccessfulIngestionAt,
        lastCycleAt: now,
...
export async function runAggregatorCycle() {
    ...
    if (evaluation.cycleStatus === "failure") {
        throw new Error(`[aggregator-cron] ingestion cycle failed: ${evaluation.reasons.join(",") || "unknown_reason"}`);
    }
}
export async function getIngestionReliabilityState() {
    const [lastSuccessfulIngestionRaw, lastCycleAtRaw, lastCycleStatusRaw, consecutiveFailureRaw, consecutiveZeroRaw, lastJobsIngestedRaw,] = await Promise.all([
    ...
    return {
        ...
        lastCycleStatus: lastCycleStatusRaw ??
            ingestionMemoryState.lastCycleStatus,
        consecutiveFailureCount: parseIntegerOrNull(consecutiveFailureRaw) ?? ingestionMemoryState.consecutiveFailureCount,
        consecutiveZeroResultCount: parseIntegerOrNull(consecutiveZeroRaw) ?? ingestionMemoryState.consecutiveZeroResultCount,
        lastJobsIngested: parseIntegerOrNull(lastJobsIngestedRaw) ?? ingestionMemoryState.lastJobsIngested,
    };
}
...
//# sourceMappingURL=aggregator-cron.js.map