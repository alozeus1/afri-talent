...
function makeCandidateToken(id = "user123") {
    return signToken({
        userId: id,
...
describe("Auth API", () => {
    ...
    describe("POST /api/auth/register", () => {
        ...
        it("returns 400 if user exists", async () => {
            prisma.user.findUnique.mockResolvedValueOnce({ id: "user123", email: "test@example.com" });
            const res = await request(app).post("/api/auth/register").send({
                email: "test@example.com",
                password: "Password123!",
                role: "CANDIDATE",
        ...
        it("registers user successfully", async () => {
            prisma.user.findUnique.mockResolvedValueOnce(null); // doesn't exist yet
