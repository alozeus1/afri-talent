...
const { prismaMock, stripeApiMock, isStripeConfiguredMock, getStripeMock, getPlanFromPriceIdMock, createUserNotificationMock, } = vi.hoisted(() => {
    const prismaMock = {
        user: {
            findUnique: vi.fn(),
            create: vi.fn(),
...
function makeToken(role = Role.CANDIDATE, userId = "user-1", email = "user@example.com") {
...
describe("Phase 1/2 quality integration suite", () => {
    beforeEach(() => {
        ...
        prismaMock.$transaction.mockImplementation(async (value) => {
            if (Array.isArray(value)) {
                return Promise.all(value);
            }
            if (typeof value === "function") {
                return value(prismaMock);
            }
            return value;
        });
    ...
    it("blocks checkout for unverified users, then creates a checkout session for verified users", async () => {
        ...
        expect(stripeApiMock.checkout.sessions.create).toHaveBeenCalledWith(expect.objectContaining({
...
//# sourceMappingURL=phase12-quality-integration.test.js.map