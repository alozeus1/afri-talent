...
import { ATSProvider, ATSConnectionStatus, Role } from "@prisma/client";
import { z } from "zod";
import prisma from "../lib/prisma.js";
import { authenticate, authorize } from "../middleware/auth.js";
import { encryptString } from "../lib/secure-string.js";
import logger from "../lib/logger.js";
import { createAtsAuditLog, getAtsConnectionDetailForEmployer, getAtsConnectionLogs, getEmployerAtsDashboard, listEmployerAtsConnections, refreshConnectionHealth, retryAtsConnectionSync, runAtsConnectionSync, runAtsConnectionTest, } from "../lib/ats/service.js";
const router = Router();
router.use(authenticate, authorize(Role.EMPLOYER));
const optionalTrimmedString = (minimumLength = 1) => z.preprocess((value) => (typeof value === "string" ? value.trim() || undefined : value), z.string().min(minimumLength).optional());
...
router.post("/connections/:id/retry", async (req, res) => {
    try {
        const employerId = await getEmployerId(req.user.userId);
        if (!employerId) {
            res.status(404).json({ error: "Employer profile not found" });
