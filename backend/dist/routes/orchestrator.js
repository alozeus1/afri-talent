...
// ── Route ─────────────────────────────────────────────────────────────────────
router.post("/run", authenticate, authorize(Role.CANDIDATE), checkDailyQuota, async (req, res) => {
    ...
    if (!parsed.success) {
        ...
        const rawBudget = req.body?.limits?.token_budget_total;
    ...
    const sendError = (httpStatus, message, stoppedReason = message) => {
    ...
    try {
        ...
        logger.error({ run_id, user_id_prefix: userId.slice(0, 8), run_type, err_msg: err.message }, "[orchestrator route] error");
...
router.get("/runs", authenticate, authorize(Role.CANDIDATE), async (req, res) => {
    try {
        const userId = req.user.userId;
