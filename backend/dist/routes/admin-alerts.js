...
router.get("/", checkPermission(AdminPermission.MANAGE_ALERTS), async (req, res) => {
    try {
...
router.get("/stats/summary", checkPermission(AdminPermission.VIEW_HEALTH_STATUS), async (_req, res) => {
    try {
        const [criticalCount, warningCount, infoCount, activeCount, resolvedLast24h, byCategoryCount,] = await Promise.all([
        ...
        res.json({
            ...
            resolvedLast24h,
            byCategory: Object.fromEntries(byCategoryCount.map((item) => [item.category, item._count])),
        });
    }
    catch (error) {
...
router.post("/:id/acknowledge", checkPermission(AdminPermission.MANAGE_ALERTS), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        await createAuditLog(req.adminRole?.id, "ALERT_CREATED", "ALERT", id, { targetName: alert.title }, req);
...
router.post("/:id/resolve", checkPermission(AdminPermission.MANAGE_ALERTS), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        await createAuditLog(req.adminRole?.id, "ALERT_CREATED", "ALERT", id, { targetName: alert.title, reason: resolution }, req);
        res.json({ success: true });
    }
    catch (error) {
...
router.get("/:id", checkPermission(AdminPermission.MANAGE_ALERTS), async (req, res) => {
    try {
        const { id } = req.params;
        const alert = await prisma.platformAlert.findUnique({ where: { id } });
        if (!alert) {
