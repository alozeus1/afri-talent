...
import { z } from "zod";
import { AbuseReportReason, AssessmentStatus, CandidateSkillVerificationMethod, CandidateSkillVerificationStatus, Role, TrustEntityType, TrustRiskLevel, VerificationArtifactType, } from "@prisma/client";
import prisma from "../lib/prisma.js";
import logger from "../lib/logger.js";
import { authenticate, authorize } from "../middleware/auth.js";
import { requireAccountStanding } from "../middleware/account-standing.js";
import { addTrustCaseAction, candidateTrustSummary, createTrustCase, employerTrustSummary, ensureCandidateTrustProfile, ensureEmployerTrustProfile, recordTrustRiskEvent, refreshCandidateTrustProfile, refreshEmployerTrustProfile, scoreDeltaForReport, trustPriorityForReport, } from "../lib/trust/service.js";
...
const reportSchema = z
    ...
    .refine((value) => Boolean(value.reportedUserId ||
    value.employerId ||
    value.targetJobId ||
...
async function deliverPhoneOtp(phoneNumber, code) {
    if (process.env.NODE_ENV === "production") {
        logger.warn({ phoneNumber }, "Phone verification requested without an SMS provider configured. Integrate Twilio or a local SMS gateway before production rollout.");
        return {};
    }
...
router.get("/employer/summary", authenticate, authorize(Role.EMPLOYER), requireAccountStanding({ allowLimited: true }), async (req, res) => {
    try {
...
router.put("/employer/profile", authenticate, authorize(Role.EMPLOYER), requireAccountStanding({ allowLimited: true }), async (req, res) => {
    try {
...
router.post("/employer/artifacts", authenticate, authorize(Role.EMPLOYER), requireAccountStanding({ allowLimited: true }), async (req, res) => {
    try {
...
router.get("/candidate/summary", authenticate, authorize(Role.CANDIDATE), requireAccountStanding({ allowLimited: true }), async (req, res) => {
    try {
...
router.post("/candidate/phone/request-otp", authenticate, authorize(Role.CANDIDATE), requireAccountStanding({ allowLimited: true }), async (req, res) => {
    try {
...
router.post("/candidate/phone/verify-otp", authenticate, authorize(Role.CANDIDATE), requireAccountStanding({ allowLimited: true }), async (req, res) => {
    try {
...
router.post("/candidate/artifacts", authenticate, authorize(Role.CANDIDATE), requireAccountStanding({ allowLimited: true }), async (req, res) => {
    try {
...
router.post("/candidate/skills", authenticate, authorize(Role.CANDIDATE), requireAccountStanding({ allowLimited: true }), async (req, res) => {
    try {
        ...
        let artifactId = null;
        let trustCaseId = null;
        let verificationStatus = CandidateSkillVerificationStatus.PENDING;
        ...
        const savedSkill = existingSkill
            ? await prisma.candidateVerifiedSkill.update({
                ...
                data: {
                    ...
                    metadata: {
                        ...(data.metadata ?? {}),
                        portfolioMatchesProfile: data.method === CandidateSkillVerificationMethod.PORTFOLIO
...
router.post("/reports", authenticate, requireAccountStanding({ allowLimited: true }), async (req, res) => {
    try {
        const data = reportSchema.parse(req.body);
        const scoreDelta = scoreDeltaForReport(data.reason);
        const priority = trustPriorityForReport(data.reason);
        let entityType = TrustEntityType.USER;
        let reportData = {
        ...
        if (data.targetJobId) {
            ...
            const reportedUserId = thread.participants.find((participant) => participant.userId !== req.user.userId)?.userId ?? null;
...
router.get("/messaging-guidance", authenticate, async (_req, res) => {
    const contentRisk = assessContentRisk("Keep conversations on-platform. Never pay fees. Do not move to WhatsApp or Telegram before trust is established.");
    res.json({
