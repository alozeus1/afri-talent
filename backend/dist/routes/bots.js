...
function secureHashEqual(hashA, hashB) {
...
router.post("/subscriptions", authenticate, subscriptionLimiter, async (req, res) => {
    try {
...
router.post("/subscriptions/:id/verify", authenticate, subscriptionLimiter, async (req, res) => {
    try {
...
router.patch("/subscriptions/:id/preferences", authenticate, async (req, res) => {
    try {
...
router.post("/subscriptions/:id/test-alert", authenticate, authorize(Role.CANDIDATE, Role.EMPLOYER), async (req, res) => {
    const subscription = await prisma.botSubscription.findFirst({
        where: {
            id: req.params.id,
            userId: req.user.userId,
...
router.post("/webhooks/:channel", subscriptionLimiter, async (req, res) => {
