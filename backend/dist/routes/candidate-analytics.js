...
router.get("/profile-views", authenticate, authorize(Role.CANDIDATE), async (req, res) => {
    try {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            const count = views.filter((v) => v.createdAt >= weekStart && v.createdAt < weekEnd).length;
            viewsByWeek.push({
                week: `Week ${i + 1}`,
                count,
            });
...
router.get("/application-funnel", authenticate, authorize(Role.CANDIDATE), async (req, res) => {
    try {
...
//# sourceMappingURL=candidate-analytics.js.map