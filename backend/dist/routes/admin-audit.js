...
router.get("/", checkPermission(AdminPermission.VIEW_AUDIT_LOGS), async (req, res) => {
    try {
        const { action, adminId, targetType, targetId, page = "1", limit = "50", startDate, endDate, } = req.query;
...
router.get("/:id", checkPermission(AdminPermission.VIEW_AUDIT_LOGS), async (req, res) => {
    try {
        const { id } = req.params;
...
router.get("/stats/summary", checkPermission(AdminPermission.VIEW_AUDIT_LOGS), async (_req, res) => {
    try {
