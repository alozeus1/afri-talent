...
router.get("/matches", authenticate, authorize(Role.CANDIDATE), requirePlan(SubscriptionPlan.PROFESSIONAL), async (req, res) => {
...
router.post("/embed-resume", authenticate, authorize(Role.CANDIDATE), requirePlan(SubscriptionPlan.PROFESSIONAL), async (req, res) => {
    if (!checkSkillsEnabled(res))
        return;
    try {
        const userId = req.user.userId;
