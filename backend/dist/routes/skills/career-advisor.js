...
router.post("/analyze", authenticate, authorize(Role.CANDIDATE), requirePlan(SubscriptionPlan.PROFESSIONAL), async (req, res) => {
...
router.get("/history", authenticate, authorize(Role.CANDIDATE), requirePlan(SubscriptionPlan.PROFESSIONAL), async (req, res) => {
    if (!checkSkillsEnabled(res))
        return;
    try {
        const userId = req.user.userId;
