...
import { translateResume } from "../../lib/ai/skills/resume-translator.js";
const router = Router();
...
router.post("/save", authenticate, authorize(Role.CANDIDATE), requirePlan(SubscriptionPlan.PROFESSIONAL), async (req, res) => {
    if (!checkSkillsEnabled(res))
        return;
    ...
    try {
        ...
        const saved = await prisma.userResume.upsert({
            where: { userId },
            create: { userId, content: content, rawText, version: 1 },
            update: {
                content: content,
                rawText,
...
router.get("/my-resume", authenticate, authorize(Role.CANDIDATE), requirePlan(SubscriptionPlan.PROFESSIONAL), async (req, res) => {
    if (!checkSkillsEnabled(res))
        return;
    try {
        const userId = req.user.userId;
...
router.post("/scan-ats", authenticate, authorize(Role.CANDIDATE), requirePlan(SubscriptionPlan.PROFESSIONAL), async (req, res) => {
    if (!checkSkillsEnabled(res))
        return;
...
router.post("/translate", authenticate, authorize(Role.CANDIDATE), requirePlan(SubscriptionPlan.PROFESSIONAL), async (req, res) => {
    if (!checkSkillsEnabled(res))
        return;
