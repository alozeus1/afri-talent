...
import { resolveCheckoutPrice, resolveUserRegion, recordBillingEvent, validateCheckoutSafety, providerSupportsPortal, resolveCheckoutProvider, } from "../lib/billing/index.js";
...
function buildFlutterwaveTxRef(userId, plan, interval) {
    return `aft_${userId}_${plan}_${interval}_${Date.now()}_${crypto.randomBytes(4).toString("hex")}`;
}
// POST /api/billing/checkout — create Stripe checkout session
router.post("/checkout", authenticate, requireVerifiedEmail(), async (req, res) => {
    ...
    try {
        ...
        const regionalPrice = await resolveCheckoutPrice(req.user.userId, plan, interval);
        // Fallback to legacy env-var prices if no regional price configured
        const priceId = regionalPrice?.stripePriceId ?? STRIPE_PRICES[plan];
        ...
        const usedLegacyFallback = providerDecision.provider === BillingProvider.STRIPE
            ...
            && !!STRIPE_PRICES[plan];
        if ((providerDecision.provider === BillingProvider.STRIPE && !priceId)
        ...
        if (providerDecision.provider === BillingProvider.FLUTTERWAVE) {
            ...
            const txRef = buildFlutterwaveTxRef(req.user.userId, plan, interval);
...
router.post("/verify-checkout", authenticate, requireVerifiedEmail(), async (req, res) => {
    try {
        ...
        const plan = metadata.plan ?? SubscriptionPlan.BASIC;
...
router.post("/portal", authenticate, requireVerifiedEmail(), async (req, res) => {
    try {
