...
import { AssessmentProvider, AssessmentStatus, CandidateSkillVerificationMethod, CandidateSkillVerificationStatus, Role, } from "@prisma/client";
...
router.get("/available", async (_req, res) => {
    try {
        ...
        const allSkills = Array.from(new Set([...COMMON_TECH_SKILLS, ...completedSkillNames])).sort();
...
router.put("/:id/complete", authenticate, async (req, res) => {
    try {
        ...
        const verificationStatus = data.score >= 70
        ...
        if (existingVerifiedSkill) {
            await prisma.candidateVerifiedSkill.update({
                ...
                data: {
                    ...
                    score: data.score,
                    confidenceNote: verificationStatus === CandidateSkillVerificationStatus.VERIFIED
                        ? "Assessment-backed verification passed the trust threshold."
                        : "Assessment result did not meet the verification threshold yet.",
                    verifiedAt: verificationStatus === CandidateSkillVerificationStatus.VERIFIED
                        ? new Date()
                        : null,
                },
            });
...
//# sourceMappingURL=skills-assessments.js.map