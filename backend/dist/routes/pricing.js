...
import { resolveUserRegion, setUserBillingCountry, getRegionalPrices, getEntitlements, getUserEntitlements, isValidCountryCode, countryToRegion, REGION_DEFAULTS, getDefaultCurrencyForCountry, } from "../lib/billing/index.js";
...
router.get("/", async (req, res) => {
    try {
        ...
        let resolvedRegion = BillingRegion.ROW;
        ...
        res.json({
            ...
            prices,
            entitlements: entitlements.reduce((acc, e) => ({ ...acc, [e.plan]: e }), {}),
        });
    }
    catch (error) {
...
router.post("/billing-country", authenticate, async (req, res) => {
    try {
        ...
        await setUserBillingCountry(req.user.userId, country, RegionChangeSource.USER_SELECTED, { ipAddress: ip });
...
router.get("/payment-localization", async (_req, res) => {
    try {
        const configs = await prisma.regionConfig.findMany({
            where: { isActive: true },
            orderBy: { region: "asc" },
...
router.get("/entitlements/:plan", async (req, res) => {
    try {
