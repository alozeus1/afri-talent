...
import { z } from "zod";
import { BillingDiscrepancyStatus, BillingDiscrepancyType, BillingSupportActionType, Role, } from "@prisma/client";
import prisma from "../lib/prisma.js";
import { authenticate, authorize } from "../middleware/auth.js";
import { createBillingSupportAction, runBillingReconciliationCycle, syncBillingEntitlementState, } from "../lib/billing/index.js";
...
const discrepancyStatusValues = ["ALL", ...Object.values(BillingDiscrepancyStatus)];
router.get("/dashboard", async (_req, res) => {
    try {
        const since = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
        const [successfulPayments, failedPayments, webhookFailures, mismatchedSubscriptionStates, unpaidButEntitled, paidButNotEntitled, pendingRefunds, recentDiscrepancies, lastReconciliationRun,] = await Promise.all([
...
router.get("/discrepancies", async (req, res) => {
    try {
        ...
        const status = discrepancyStatusValues.includes(rawStatus)
            ? rawStatus
            : "OPEN";
...
router.post("/customers/:userId/resync-entitlements", async (req, res) => {
    try {
        const customer = await prisma.user.findUnique({
            where: { id: req.params.userId },
            select: {
...
router.post("/customers/:userId/actions", async (req, res) => {
    try {
...
//# sourceMappingURL=admin-billing.js.map