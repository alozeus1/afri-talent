...
import { z } from "zod";
import { AbuseReportStatus, CandidateSkillVerificationMethod, CandidateSkillVerificationStatus, JobStatus, ModerationActionType, Role, TrustEntityType, TrustCaseStatus, TrustRiskLevel, VerificationArtifactStatus, VerificationArtifactType, } from "@prisma/client";
import prisma from "../lib/prisma.js";
import logger from "../lib/logger.js";
import { authenticate, authorize } from "../middleware/auth.js";
import { addTrustCaseAction, createTrustCase, refreshCandidateTrustProfile, refreshEmployerTrustProfile, } from "../lib/trust/service.js";
...
function inferCaseStatus(actionType) {
    switch (actionType) {
...
async function resolveCaseTargetUser(caseRecord) {
    if (!caseRecord) {
        return null;
    }
...
async function applyCaseActionEffects(caseRecord, action) {
    if (!caseRecord)
        return;
...
router.get("/dashboard", async (_req, res) => {
    try {
        const [pendingVerificationArtifacts, openRiskCases, openReports, limitedAccounts, suspendedAccounts, heldJobs,] = await Promise.all([
...
router.post("/artifacts/:id/review", async (req, res) => {
    try {
        ...
        const trustCase = artifact.trustCase ??
        ...
        await addTrustCaseAction({
            ...
            actionType: data.status === VerificationArtifactStatus.APPROVED
        ...
        await prisma.trustCase.update({
            ...
            data: {
                status: data.status === VerificationArtifactStatus.NEEDS_MORE_INFO
        ...
        if (artifact.employerId) {
            ...
            if (artifact.employer?.userId) {
                await createUserNotification({
                    ...
                    type: "VERIFICATION",
                    title: data.status === VerificationArtifactStatus.APPROVED
                    ...
                    body: data.status === VerificationArtifactStatus.APPROVED
            ...
            if (artifact.employer) {
                ...
                if (employerUser?.email) {
                    await verificationEmail({
                        ...
                        status: data.status === VerificationArtifactStatus.APPROVED
                            ? "VERIFIED"
                            : data.status === VerificationArtifactStatus.NEEDS_MORE_INFO
                                ? "PENDING"
                                : "UNVERIFIED",
        ...
        if (artifact.userId) {
            if (artifact.type === VerificationArtifactType.CERTIFICATION) {
                const skillStatus = data.status === VerificationArtifactStatus.APPROVED
                ...
                await prisma.candidateVerifiedSkill.updateMany({
                    ...
                    data: {
                        ...
                        verifiedAt: data.status === VerificationArtifactStatus.APPROVED
                            ? new Date()
                            : null,
                    },
                });
                    ...
                    if (!linkedSkill) {
                        await prisma.candidateVerifiedSkill.create({
                            data: {
                                ...
                                artifactId: artifact.id,
                                reviewerId: req.user.userId,
                                evidenceLabel: typeof metadata?.evidenceLabel === "string" ? metadata.evidenceLabel : null,
...
router.get("/risk-queue", async (req, res) => {
    try {
    ...
    try {
        ...
        const updatedCase = await prisma.trustCase.update({
            ...
            data: {
                ...
                assignedAdminId: req.user.userId,
                closedAt: (data.status ?? inferCaseStatus(data.actionType)) === TrustCaseStatus.ACTIONED ||
