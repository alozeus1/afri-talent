...
import { EMPLOYER_ONBOARDING_STEPS, buildEmployerOnboardingChecklist, completionPercentage, employerUpgradeJourney, hasCandidateFilters, isJobApproved, isPlanSelectionDone, milestoneStatus, nextEmployerAction, normalizeEmployerCandidateFilters, qualifiedApplicantsCount, timeToFirstQualifiedApplicantHours, verificationImpactSummary, verificationIsStrong, } from "../lib/employer/onboarding.js";
import { evaluateJobQuality } from "../lib/jobs/discovery.js";
import { assessJobPostingRisk } from "../lib/trust/risk.js";
import { employerTrustSummary, refreshEmployerTrustProfile, } from "../lib/trust/service.js";
const router = Router();
...
async function getEmployerOr404(res, userId) {
...
function normalizeStatusCounts(groups) {
    return groups.reduce((acc, item) => {
        acc[item.status] = item._count.id;
        return acc;
    }, {});
}
function analyticsPropertyString(value, key) {
...
async function buildEmployerActivationData(userId, employerId) {
    const [trustProfile, subscription, jobs, applications, candidateViewEvent, recentApplications, statusGroups] = await Promise.all([
    ...
    const shortlistedApplications = applications.filter((item) => item.status === "SHORTLISTED" || item.status === "ACCEPTED");
...
router.get("/analytics", authenticate, authorize(Role.EMPLOYER), async (req, res) => {
    try {
        ...
        res.json({
            ...
            shortlistRate: activation.applications.length > 0
...
router.get("/branding", authenticate, authorize(Role.EMPLOYER), async (req, res) => {
    try {
        const employer = await prisma.employer.findUnique({
...
router.put("/branding", authenticate, authorize(Role.EMPLOYER), async (req, res) => {
    try {
...
router.get("/onboarding", authenticate, authorize(Role.EMPLOYER), async (req, res) => {
    try {
        const employer = await getEmployerOr404(res, req.user.userId);
        if (!employer)
            return;
        ...
        const checklist = buildEmployerOnboardingChecklist({
            ...
            subscriptionChoiceDone: isPlanSelectionDone(activation.subscription?.plan ?? SubscriptionPlan.EMPLOYER_FREE, employer.onboardingState?.selectedPlan ?? null),
            trustCompletionDone: activation.trust.checklist.every((item) => item.done) ||
        ...
        res.json({
            ...
            nextAction,
            recommendation: activation.milestones.firstQualifiedShortlistAt
...
router.put("/onboarding", authenticate, authorize(Role.EMPLOYER), async (req, res) => {
    try {
        const employer = await getEmployerOr404(res, req.user.userId);
        if (!employer)
            return;
        ...
        const onboardingState = await prisma.$transaction(async (tx) => {
            ...
            return tx.employerOnboardingState.upsert({
                ...
                create: {
                    ...
                    candidateFilters: (data.candidateFilters ?? undefined),
...
router.post("/onboarding/job-preview", authenticate, authorize(Role.EMPLOYER), async (req, res) => {
    try {
        const employer = await getEmployerOr404(res, req.user.userId);
        if (!employer)
            return;
        ...
        const strengths = [];
        const tips = [];
        if (quality.signals.descriptionComplete)
            strengths.push("Job scope is detailed enough to build candidate confidence.");
        else
            tips.push("Add responsibilities, outcomes, and requirements so candidates can self-qualify faster.");
        if (quality.signals.compensationTransparent)
            strengths.push("Compensation transparency improves credibility and apply quality.");
        else
            tips.push("Add a salary range to increase trust and reduce low-fit applications.");
        if (quality.signals.locationClear)
            strengths.push("Location is clear for cross-border candidates.");
        else
            tips.push("Clarify remote, hybrid, or onsite expectations and include country or timezone detail.");
        if (quality.signals.mobilityClear)
            strengths.push("Visa or relocation expectations are clear.");
        else
            tips.push("Clarify visa sponsorship, relocation support, or eligible hiring countries.");
        if (quality.signals.verifiedEmployer)
            strengths.push("Employer verification is helping job credibility.");
        else
            tips.push("Complete employer verification to reduce moderation friction and improve trust.");
        const moderationRequired = trustProfile.requiresEnhancedVerification ||
            risk.autoHold ||
            risk.level === "HIGH" ||
            risk.level === "CRITICAL";
        res.json({
...
router.get("/analytics/advanced", authenticate, authorize(Role.EMPLOYER), async (req, res) => {
    try {
        const employer = await getEmployerOr404(res, req.user.userId);
        if (!employer)
            return;
        ...
        const postingPerformance = activation.jobs.map((job) => {
            ...
            const sortedByCreated = [...jobApplications].sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime());
            const firstApplicationAt = sortedByCreated[0]?.createdAt || null;
            const timeToFirstApplicationHours = firstApplicationAt && job.publishedAt
                ? Number(((firstApplicationAt.getTime() - job.publishedAt.getTime()) / (1000 * 60 * 60)).toFixed(2))
                : null;
            ...
            return {
                ...
                qualifiedApplicants,
                shortlistRate: jobApplications.length > 0
                    ? Number(((qualifiedApplicants / jobApplications.length) * 100).toFixed(2))
                    : 0,
                acceptanceRate: jobApplications.length > 0
                    ? Number((((jobApplications.filter((item) => item.status === "ACCEPTED").length) / jobApplications.length) * 100).toFixed(2))
                    : 0,
        ...
        const avgReviewTimeHours = reviewedDurationsHours.length > 0
            ? Number((reviewedDurationsHours.reduce((sum, value) => sum + value, 0) / reviewedDurationsHours.length).toFixed(2))
            : null;
        ...
        const averageJobQuality = postingPerformance.length > 0
        ...
        res.json({
            funnel: [
                ...
                ...row,
                conversionFromPrevious: index === 0 || arr[index - 1].count === 0
                    ? null
            ...
            roi: {
                ...
                shortlistRate: activation.applications.length > 0
                    ? Number(((activation.qualifiedApplicants / activation.applications.length) * 100).toFixed(2))
                    : 0,
                clickToApplyRate: totalClicks > 0
...
//# sourceMappingURL=employer-analytics.js.map