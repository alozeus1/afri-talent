...
router.put("/", authenticate, authorize(Role.CANDIDATE), async (req, res) => {
    try {
...
router.get("/resumes", authenticate, authorize(Role.CANDIDATE), async (req, res) => {
    try {
        const profile = await prisma.candidateProfile.findUnique({
            where: { userId: req.user.userId },
            select: { id: true },
...
router.post("/resumes", authenticate, authorize(Role.CANDIDATE), async (req, res) => {
    try {
...
router.get("/analytics", authenticate, authorize(Role.CANDIDATE), async (req, res) => {
    try {
        const profile = await prisma.candidateProfile.findUnique({
            where: { userId: req.user.userId },
            select: { id: true },
