...
router.post("/negotiate", authenticate, authorize(Role.CANDIDATE), makeMonthlySkillQuota(AiRunType.SALARY_NEGOTIATE, 3), async (req, res) => {
    if (!checkSkillsEnabled(res))
        return;
    try {
        const { role, location, currentSalary, targetSalary, currency, yearsExperience } = negotiateSchema.parse(req.body);
...
//# sourceMappingURL=salary-benchmarks.js.map