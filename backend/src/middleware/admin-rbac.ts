 ...
 */
export async function enforceAdminRbac(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
 ...
 */
export function checkPermission(requiredPermission: AdminPermission) {
    return (req: Request, res: Response, next: NextFunction) => {
        if (!req.adminPermissions?.includes(requiredPermission)) {
            logger.warn(
                `User ${req.user?.userId} lacks permission ${requiredPermission}`
            );
            res.status(403).json({
 ...
 */
export function checkAnyPermission(requiredPermissions: AdminPermission[]) {
    return (req: Request, res: Response, next: NextFunction) => {
        const hasPermission = requiredPermissions.some((perm) =>
            req.adminPermissions?.includes(perm)
        );

        if (!hasPermission) {
            logger.warn(
                `User ${req.user?.userId} lacks any of permissions: ${requiredPermissions.join(", ")}`
            );
            res.status(403).json({
