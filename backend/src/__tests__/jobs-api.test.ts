...
function makeEmployerToken(id = "emp123"): string {
    return signToken({
        userId: id,
...
describe("Jobs API", () => {
    ...
    describe("GET /api/jobs/:slug", () => {
        it("returns 404 for non-existent job", async () => {
            (prisma.job.findUnique as any).mockResolvedValueOnce(null);
        ...
        it("returns 200 for existing published job", async () => {
            (prisma.job.findUnique as any).mockResolvedValueOnce(DUMMY_JOB);
    ...
    describe("PUT /api/jobs/:id", () => {
        ...
        it("updates job successfully", async () => {
            ...
            // Perform update
            (prisma.job.update as any).mockResolvedValueOnce({ ...DUMMY_JOB, title: "Updated Title" });
