...
const SUCCESSFUL_PAYMENT_EVENTS = ["invoice.paid", "invoice.payment_succeeded"] as const;
const FAILED_PAYMENT_EVENTS = ["invoice.payment_failed"] as const;
...
function hashEntitlements(value: unknown): string {
...
function isEntitlementGrantingStatus(status: SubscriptionStatus): boolean {
  return status !== SubscriptionStatus.CANCELLED && status !== SubscriptionStatus.INACTIVE;
}

async function buildExpectedBillingState(userId: string): Promise<BillingExpectation> {
...
export async function validateBillingEntitlementState(userId: string): Promise<BillingStateValidation> {
...
export async function upsertBillingDiscrepancy(input: BillingDiscrepancyInput) {
...
export async function resolveBillingDiscrepancy(discrepancyId: string) {
  return prisma.billingDiscrepancy.update({
...
export async function syncBillingEntitlementState(
  userId: string,
  source = "system",
) {
...
export async function validateCheckoutSafety(userId: string) {
  ...
  if (!profile) {
    return { allowed: true, profile: null as typeof profile };
  }
...
async function compareRemoteSubscription(
  runId: string,
  subscription: {
    id: string;
    userId: string;
...
export async function runBillingReconciliationCycle(triggeredBy = "scheduler"): Promise<BillingReconciliationResult> {
