...
const QUANTIFICATION_PATTERN =
  /\b\d{1,3}(?:[.,]\d+)?(?:\s?%| percent|\+|k|K|M|bn|BN|\s?(?:users|customers|clients|dollars|USD|EUR|GBP|₦|NGN|ZAR|KES|months|years|weeks|days))\b/;

export interface RubricInputs {
  text: string;
  kind: "resume" | "cover_letter" | "match_explanation" | "generic";
  // Optional target-role keywords to check presence of.
...
function collectBannedHits(text: string): string[] {
  const hits: string[] = [];
...
function collectFillerHits(text: string): string[] {
  const hits: string[] = [];
  const hay = text.toLowerCase();
...
function collectPlaceholderHits(text: string): string[] {
  const hits: string[] = [];
...
function countQuantifiedAchievements(text: string): number {
  ...
  for (const s of sentences) {
    if (QUANTIFICATION_PATTERN.test(s)) count++;
  }
  return count;
}
 ...
 */
export function gradeAiOutput(input: RubricInputs): QualityReport {
  ...
  for (const issue of issues) {
    ...
    else if (issue.severity === "warn") score -= 7;
    else score -= 2;
  }
  if (score < 0) score = 0;
  ...
  const grade: QualityGrade = hasError || score < 55 ? "FAIL" : score < 80 ? "WARN" : "PASS";
 ...
 */
export function formatReportLine(report: QualityReport): string {
