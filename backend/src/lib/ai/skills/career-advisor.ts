...
export interface CareerAdvisorInput {
  ...
  recentJobTitles: string[]; // from application history
}

export interface CareerAdviceResult {
  skillGaps: Array<{ skill: string; priority: "high" | "medium" | "low"; reason: string }>;
  careerPaths: Array<{ title: string; timeline: string; description: string }>;
...
export async function analyseCareer(input: CareerAdvisorInput): Promise<CareerAdviceResult> {
...
async function analyseWithClaude(input: CareerAdvisorInput): Promise<CareerAdviceResult> {
...
- Action plan should be specific and immediately actionable`;
  ...
  return JSON.parse(jsonStr) as CareerAdviceResult;
}

function buildTemplateAdvice(input: CareerAdvisorInput): CareerAdviceResult {
  return {
    skillGaps: [
      {
