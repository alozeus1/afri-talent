...
export const semanticCandidateSelect = Prisma.validator<Prisma.CandidateProfileSelect>()({
  id: true,
  userId: true,
  headline: true,
  bio: true,
...
function stringifyHistory(value: unknown, fields: string[]): string[] {
...
export function isSemanticCandidateIndexable(candidate: SemanticCandidateRecord): boolean {
  return candidate.openToWork && Boolean(candidate.userId);
}

export function buildCandidateSemanticContent(candidate: SemanticCandidateRecord): string {
  const sections = [
    candidate.user.name,
    candidate.headline,
    candidate.bio,
...
export function buildCandidateSemanticMetadata(candidate: SemanticCandidateRecord): Record<string, unknown> {
  return {
