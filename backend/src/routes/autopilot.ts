...
import {
  ...
  buildResumeVersion,
  buildTaskTitle,
  ensureCandidateAutopilotProfile,
  getCandidatePipelineSnapshot,
  queueCandidateAgentTask,
} from "../lib/autopilot/framework.js";
...
function featureBlocked(res: Response, message: string) {
...
function buildRecommendations(input: {
  ...
  taskSummary: { queued: number; needsReview: number; ready: number; completed: number };
  latestResumeVersionTitle?: string | null;
}): string[] {
  const recs: string[] = [];
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      let followUps: Awaited<ReturnType<typeof buildFollowUpDraft>> = [];
      let interviewPrepTaskId: string | null = null;
