...
import { AdminPermission, BulkOperationType, BulkOperationStatus, Role, AccountRestrictionStatus } from "@prisma/client";
...
router.post(
    "/",
    checkPermission(AdminPermission.RUN_BULK_OPERATIONS),
        ...
        try {
            ...
            if (modifyingOps.includes(operationType)) {
                ...
                await createAuditLog(
                    ...
                    "BULK_OPERATION",
                    operation.id,
                    { targetName: operationType, reason, userCount: userIds?.length },
                    req
                );
            ...
            // Execute operation
            let result: any = { success: false, error: "Operation type not supported" };
            ...
            if (result?.success) {
                ...
                await createAuditLog(
                    ...
                    "BULK_OPERATION",
                    operation.id,
                    { targetName: operationType, successCount: result.count },
                    req
                );
            } else {
                await prisma.bulkOperation.update({
...
async function executeBulkCustomQuery(
    operationId: string,
    filters: any
): Promise<{ success: boolean; count?: number; data?: any; error?: string }> {
    try {
