...
import {
  MockInterviewArtifactType,
  MockInterviewStatus,
  MockInterviewVisibility,
  Prisma,
  Role,
  SupportedLocale,
} from "@prisma/client";
...
function summarizeFeedback(input: {
  ...
  notes?: string[];
}): { summary: string; overall: number | null } {
  const scores = [
  ...
  if (typeof input.scoreCommunication === "number") {
    if (input.scoreCommunication >= 75) strengths.push("clear communication");
    if (input.scoreCommunication < 60) improvements.push("concise communication");
  }
  if (typeof input.scoreTechnical === "number") {
    if (input.scoreTechnical >= 75) strengths.push("technical depth");
    if (input.scoreTechnical < 60) improvements.push("technical accuracy");
  }
  if (typeof input.scoreProblemSolving === "number") {
    if (input.scoreProblemSolving >= 75) strengths.push("structured problem solving");
    if (input.scoreProblemSolving < 60) improvements.push("problem-solving structure");
  }
...
router.get("/:id", authenticate, authorize(Role.CANDIDATE), async (req: Request, res: Response) => {
...
router.post("/:id/feedback", authenticate, authorize(Role.CANDIDATE), async (req: Request, res: Response) => {
  try {
...
router.post("/:id/privacy", authenticate, authorize(Role.CANDIDATE), async (req: Request, res: Response) => {
  try {
...
router.post("/:id/artifacts", authenticate, authorize(Role.CANDIDATE), async (req: Request, res: Response) => {
  try {
...
router.post("/generate", authenticate, authorize(Role.CANDIDATE), async (req: Request, res: Response) => {
  try {
...
router.post("/:id/submit-answer", authenticate, authorize(Role.CANDIDATE), async (req: Request, res: Response) => {
  try {
