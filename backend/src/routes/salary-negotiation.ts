...
router.post("/sessions", negotiationLimiter, async (req: Request, res: Response) => {
  try {
    ...
    const marketWhere = {
      jobTitle: { contains: data.jobTitle, mode: "insensitive" as const },
      ...(data.country ? { country: { contains: data.country, mode: "insensitive" as const } } : {}),
    };
    ...
    if (!marketMedian) riskFlags.push("Limited market data for this role/country; validate with 2-3 external sources.");
