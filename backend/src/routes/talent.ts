...
async function getEmployerContext(userId: string) {
...
async function loadCandidateEvidence(userIds: string[], employerId?: string | null) {
  if (userIds.length === 0) {
    return {
      assessments: new Map<string, any[]>(),
      verifiedSkills: new Map<string, any[]>(),
      partnerMarkers: new Map<string, any[]>(),
      immigrationProcesses: new Map<string, any[]>(),
      poolMemberships: new Map<string, any[]>(),
    };
  }
  ...
  const assessmentsMap = new Map<string, typeof assessments>();
  ...
  const verifiedSkillsMap = new Map<string, typeof verifiedSkills>();
  ...
  const immigrationMap = new Map<string, typeof immigrationProcesses>();
  ...
  const poolMembershipMap = new Map<string, typeof poolMemberships>();
...
function buildTalentResult(params: {
  candidate: Prisma.CandidateProfileGetPayload<{
    include: {
      user: {
        select: {
          id: true;
  ...
  const baseMatch = buildTalentMatch({
    ...
    candidate: {
      ...
      workHistory: Array.isArray(params.candidate.workHistory)
        ? params.candidate.workHistory as Array<{ company?: string | null; title?: string | null; description?: string | null; period?: string | null }>
        : [],
    },
...
router.post("/pools/:poolId/candidates", async (req: Request, res: Response) => {
  try {
...
router.patch("/pools/:poolId/candidates/:candidateUserId", async (req: Request, res: Response) => {
  try {
...
router.delete("/pools/:poolId/candidates/:candidateUserId", async (req: Request, res: Response) => {
  try {
    const employer = await getEmployerContext(req.user!.userId);
    if (!employer) {
      res.status(404).json({ error: "Employer profile not found" });
...
router.get("/compare", async (req: Request, res: Response) => {
  try {
    ...
    const candidateIds = Array.from(new Set(
      payload.candidateIds.split(",").map((id) => id.trim()).filter(Boolean),
    )).slice(0, 4);
    ...
    const commonSkills = enriched.reduce<string[]>((shared, candidate, index) => {
      if (index === 0) {
...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    const requiresPremiumTrustFilters =
      parseBoolean(parsed.verifiedOnly) || parseBoolean(parsed.verifiedSkillsOnly) || parseBoolean(parsed.assessmentBackedOnly);
    ...
    if (parsed.query) {
      ...
      const queryClauses: Prisma.CandidateProfileWhereInput[] = [
